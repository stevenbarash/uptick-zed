...
| Release | Highlight |
|---|---|
| [**v0.6.2**](https://github.com/stevenbarash/uptick-zed/releases/tag/v0.6.2) | Bump surfaces. Inlay marks major-version deltas with `⚠ major`. New aggregate `Bump N deps to latest (safe)` quickfix bundles every patch- / minor-only candidate into a single edit. File-top summary lens reports `⛔ N vulnerabilities · → M outdated` and jumps to the highest-severity advisory. |
| [**v0.6.1**](https://github.com/stevenbarash/uptick-zed/releases/tag/v0.6.1) | Onboarding polish. First-run welcome toast (once per user, OS-scoped state flag) with an "Open README" action that fires `window/showDocument`. `$/progress` banner is now a live counter — emits `done/total` Report updates as each registry fetch completes. README gains a "First run" section documenting the two Zed settings worth enabling (`code_lens: "on"`, `auto_install_extensions`). |
